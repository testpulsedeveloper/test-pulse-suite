...
const extractFunction = (code, funcName) => {
    const startStr1 = `const ${funcName} = () => {`;
    const startStr2 = `const ${funcName} = () => (`;
    ...
    if (startIndex === -1) {
        startIndex = code.indexOf(startStr2);
        openChar = '(';
        closeChar = ')';
    }
    ...
    for (let i = startIndex; i < code.length; i++) {
        ...
        // Handling the fact that renderConfigTab might mix ( and {
        // ACTUALLY, a much simpler approach: just find the index and then we know exactly what we're replacing.
    }
...
// SIMPLER: Regex
const replaceReports = (code, newRep) => {
  const match = code.match(/const renderReportsTab = \(\) => \{[\s\S]*?\n  \};\n/);
  if(match) return code.replace(match[0], newRep + '\n');
  return code;
}

const replaceConfig = (code, newConf) => {
  const match = code.match(/const renderConfigTab = \(\) => \([\s\S]*?\n  \);\n/);
  if(match) return code.replace(match[0], newConf + '\n');
  return code;
}
...
newCode = replaceReports(newCode, newReports);
newCode = replaceConfig(newCode, newConfig);

fs.writeFileSync('static/hello-world/src/App.js', newCode);
