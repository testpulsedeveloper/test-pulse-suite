...
async function fetchAllIssues(jql, fields, expand, properties, maxPages = 30) {
  try {
    let maxResults = 100;
    ...
    const expandStr = expand ? (Array.isArray(expand) ? expand.join(',') : expand) : '';
    const propStr = properties ? (Array.isArray(properties) ? properties.join(',') : properties) : '';

    // 1. Fetch first page to get total
    const firstRes = await api.asUser().requestJira(route`/rest/api/3/search?jql=${jql}&maxResults=${maxResults}&startAt=0&fields=${fieldsStr}&expand=${expandStr}&properties=${propStr}`, {
      method: 'GET',
    ...
    if (total > maxResults) {
        ...
        for (let i = 0; i < offsets.length; i += BATCH_SIZE) {
            ...
            const promises = batch.map(async (startAt) => {
                const res = await api.asUser().requestJira(route`/rest/api/3/search?jql=${jql}&maxResults=${maxResults}&startAt=${startAt}&fields=${fieldsStr}&expand=${expandStr}&properties=${propStr}`, {
                  method: 'GET',
                  headers: { 'Accept': 'application/json' }
                });
                if (res.ok) {
